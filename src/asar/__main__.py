"""Handling python -m asar command."""
...
def _create_pack_handler(parser: ArgumentParser) -> ArgumentParser:
    ...
    _ = parser.add_argument(
        "--ordering",
        type=Path,
        help="path to a text file for ordering contents",
    )
    _ = parser.add_argument(
        "--unpack",
        help="do not pack files matching glob expression <UNPACK>",
    )
    _ = parser.add_argument(
        "--unpack-dir",
        help=(
            "do not pack dirs matching glob expression <UNPACK_DIR>"
            " or starting with literal expression <UNPACK_DIR>"
        ),
    )
    _ = parser.add_argument(
        "--exclude-hidden",
